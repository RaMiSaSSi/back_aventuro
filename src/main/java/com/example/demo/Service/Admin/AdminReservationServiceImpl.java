// java
        ...
        public class AdminReservationServiceImpl implements AdminReservationService {
            ...
            private Reservation dtoToEntity(ReservationDTO dto, Reservation existing) {
                ...
                // user optional: if provided link to user, else clear user and use manual client fields
                if (dto.getUserId() != null) {
                    UtilisateurInscrit user = utilisateurRepository.findById(dto.getUserId())
                            .orElseThrow(() -> new RuntimeException("Utilisateur non trouvé"));
                    r.setUser(user);
                    // clear manual client fields when a registered user is used (optional)
                    r.setClientNom(null);
                ...
                // set creation date if missing (only set on create)
                if (existing == null) {
